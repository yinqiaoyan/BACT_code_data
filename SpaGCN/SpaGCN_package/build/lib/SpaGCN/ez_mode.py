...
# 		1. input of the function calculate_adj_matrix
#		2. clf.train: init=init_method, n_clusters=n_clusters (for sklearn.cluster.kmeans)
# 	"calculate_adj_matrix": add input parameter platform=["ST", "Visium"]
# add functions:
#	"detect_spatial_domains_ez_mode_input_g": for simulation
################

def detect_spatial_domains_ez_mode(adata, img, x_array, y_array, x_pixel, y_pixel, n_clusters, histology=True, s=1, b=49, p=0.5,
	r_seed=100, t_seed=100, n_seed=100, max_epochs=200, init_method="kmeans", platform="ST", num_pcs=50):  # init_method="kmeans", "louvain"
	# adj=calculate_adj_matrix(x=x_pixel,y=y_pixel, x_pixel=x_pixel, y_pixel=y_pixel, image=img, beta=b, alpha=s, histology=histology)  # Wrong code!!!
	adj=calculate_adj_matrix(x=x_array,y=y_array, x_pixel=x_pixel, y_pixel=y_pixel, image=img, beta=b, alpha=s, histology=histology, platform=platform)
	...
	clf.train(adata, adj, num_pcs=num_pcs, init_spa=True, init=init_method, n_clusters=n_clusters, res=res, tol=5e-3, lr=0.05, max_epochs=max_epochs)
	y_pred, prob=clf.predict()
	return y_pred

def detect_spatial_domains_ez_mode_input_g(adata, img_g, x_array, y_array, n_clusters, histology=True, s=1, p=0.5, 
	r_seed=100, t_seed=100, n_seed=100, max_epochs=200, init_method="kmeans", num_pcs=50):
	...
	# Wrong Code!!! It should be "x=x_array, y=y_array, x_pixel=x_pixel, y_pixel=y_pixel" !!!
	adj=calculate_adj_matrix_input_g(x=x_array, y=y_array, g=img_g, alpha=s, histology=histology)
	# prefilter_genes(adata,min_cells=3) # avoiding all genes are zeros
	# prefilter_specialgenes(adata)
	# sc.pp.normalize_per_cell(adata)
	# sc.pp.log1p(adata)
